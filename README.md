...
Source code for the personal academic homepage, an English-first static website with an independent Chinese résumé. Hosted in the `yangyang-li/Homepage` repository and published to GitHub Pages (currently at `https://yangyang-li.github.io/Homepage/`).

The site was rebuilt from a legacy WordPress installation and uses the thin-starter release of [al-folio](https://github.com/alshedivat/al-folio), publishing through GitHub Actions. A lightweight browser-based CMS (Sveltia CMS) is available at `/admin/` for day-to-day content edits; see `docs/使用说明.md` (in Chinese) for the update workflow.
...
The recovered original WordPress content is recorded in `_archive/original-wordpress-site.md` (not published).
